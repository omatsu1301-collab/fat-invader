...
1. Art inventoryとasset pipelineを確定。詳細は `docs/09_ART_ASSET_INVENTORY.md`。**本stepは部分完了:** Art inventory自体、素材制作方針(強化placeholder先行)、texture/audio keyの差し替え可能設計方針は確定済み。ラスター向け Pixel Asset Spec / Style North Star / manifest schema 草案は `docs/10_PIXEL_ASSET_SPEC.md`（FI-10）。画像未生成・Human Gate待ち。asset-manifest.ts 実体、素材license記入、BGM/SE音源供給方法は未確定で、step 7(Sound mixとvoice limit)着手直前までに確定すればよい。この未確定分がstep 2〜6の着手を妨げることはない。Style North Star 承認前に素材量産へ進まない。
