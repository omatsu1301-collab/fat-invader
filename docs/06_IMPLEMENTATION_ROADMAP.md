...
1. Art inventoryとasset pipelineを確定。詳細は `docs/09_ART_ASSET_INVENTORY.md` / `docs/10_PIXEL_ASSET_SPEC.md`。**前進:** North Star 5 still concepts Human APPROVED（2026-09-10）。決定論的 raw→master→processed→public と `asset-manifest.ts` を Draft 統合。AC-232/233 は runtime Human Gate 待ち。BGM/SE音源は未確定。animation / 残り Player tier 量産はしない。
