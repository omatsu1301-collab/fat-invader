...
4. Player hit / FAT OVERを完成。**本stepは実装済み (Human Gate 2 follow-up: 表示時間 約1.6s→約2.6s):** 被弾vignette / shake / FAT OVER squash pose + caption。`fatOverHoldMs: 2600`。BGM/SEなし。
...
| FAT OVER pose | 余韻をもっと長く（2026-09-09） | 約1.6s → 約2.6s（`fatOverHoldMs: 2600`）。squash pose +「FAT OVER / 満腹につき、いったん帰還。」をResult前に維持 |
